...
class kv450_single_likelihood(Likelihood):

    def __init__(self, path, data, command_line):

        Likelihood.__init__(self, path, data, command_line)

        # root directory for the data vector
        self.data_directory = data.path['data'] +'/KV450_' + self.sample_tag
        ...
        if self.use_cterm_function:
            fname = os.path.join(self.data_directory, 'supplement/KV450_xi_pm_c_term.dat')
        ...
        for zbin in xrange(self.nzbins):
            window_file_path = os.path.join(
                self.data_directory, 'redshift/Nz_{0:}/Nz_{0:}_Mean/Nz_{0:}_z{1:}.asc'.format(self.nz_method, self.zbin_labels[zbin]))
        ...
        print('Loaded redshift distributions from: \n', os.path.join(
                self.data_directory, 'redshift/Nz_{0:}/Nz_{0:}_Mean/'.format(self.nz_method)), '\n')
    ...
    def __load_public_data_vector(self):
        ...
        for zbin1 in xrange(self.nzbins):
            for zbin2 in xrange(zbin1, self.nzbins):
                ...
                fname = os.path.join(self.data_directory, 'data_vector/xi_for_cosmo_tomo_{:}_{:}_withK_{:}.dat'.format(zbin1 + 1, zbin2 + 1, self.sample_tag))
        ...
        print('Loaded data vectors from: \n', os.path.join(self.data_directory, 'data_vector/'), '\n')
    ...
    def __load_public_cov_mat(self):
        ...
        if self.sample_tag != 'whole':
            ...
            raise Exception("Unsupported sample_tag value !")
        ...
        if (self.shift_n_z_by_D_z.any()):
            ...
            for zbin in xrange(self.nzbins):
                ...
                if param_name in data.mcmc_parameters:
                    ...
                    z_mod = self.z_p
                # Load n(z) again:
                fname = os.path.join(self.data_directory, 'redshift/Nz_{0:}/Nz_{0:}_Mean/Nz_{0:}_z{1:}.asc'.format(self.nz_method, self.zbin_labels[zbin]))
